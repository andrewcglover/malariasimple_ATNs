## =====================================================================
## malariasimple_aitn_stochastic.R
##
## Stochastic (individual-level, Binomial-draw) version of the AITN
## (antimalarial-treated net) model.
##
## Built as the analogue of malariasimple_stochastic.R, applied to the
## extended ATN mosquito model of malariasimple_aitn_deterministic_v3.R.
##
## Relationship to the existing files (mirrors deterministic -> stochastic):
##   * HUMAN disease dynamics (S,T,D,A,U,P transitions, births and SMC
##     infection-clearing) are STOCHASTIC: per-capita rates are turned into
##     per-timestep probabilities and individuals are drawn with Binomial()
##     (a sequence of conditional Binomials partitions leavers between
##     destinations, since odin/dust has no Multinomial).
##   * The MOSQUITO model (extended ATN Sv/Ev/Iv compartments, Hill kernels,
##     Bompard TRA->TBA, post-infection blocking, multi-event ATN coverage),
##     the LARVAL model, the immunity states and the FOI/EIP lags remain
##     DETERMINISTIC -- exactly as the original stochastic model leaves the
##     mosquito/larval dynamics deterministic. EIR therefore uses Ivtot.
## =====================================================================
