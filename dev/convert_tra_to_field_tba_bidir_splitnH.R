...
# "bidir_split").  OLRE / mu priors do not affect b(s) and are excluded.
# The same lab-scale prior on b(s) applies to TRA and TBA fits.
...
df_sens <- bind_rows(
  df_tba_low   %>% mutate(label = sprintf("m = %.4g (1/4 x central)", m_low)),
  df_tba_field %>% mutate(label = sprintf("m = %.4g (central)",       m_central)),
  df_tba_high  %>% mutate(label = sprintf("m = %.4g (2 x central)",   m_high))
)
...
panel_labels <- c("Lab TRA"   = "Lab TRA (%)",
                  "Field TBA" = "Field TBA (%)",
                  "Lab TBA"   = "Lab TBA (%)")
...
ggsave("atn_main_bidir_splitnH_three_curve_comparison_with_data.png", p_facet,
       width = 8, height = 11, dpi = 450, bg = "transparent")
