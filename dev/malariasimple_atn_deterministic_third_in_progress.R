...
# deltar <- parameter()
# deltarp1 <- deltar + 1 # ATN-exposed compartments plus non-exposed

# Number of ATN-exposed compartments (compile-time constants)
deltar     <- parameter(type = "integer", constant = TRUE)
deltarp1 <- parameter(type = "integer", constant = TRUE)
...
initial(Ev[2:deltarp1,]) <- 0
initial(Iv[1]) <- init_Iv * mv0
initial(Iv[2:deltarp1]) <- 0
...
dim(dSv) <- deltarp1
dSv[1] <- dt * (betaa + kappa*Sv[deltarp1] - (av*delta_atn + (1-delta_atn)*FOIvdel + mu)*Sv[1])
dSv[2] <- dt * (delta_atn*(av-FOIvdel)*Svtot - (av*delta_atn + (1-delta_atn)*FOIvdel + kappa + mu)*Sv[2])
dSv[3:deltarp1] <- dt * (kappa*Sv[i-1] - (av*delta_atn + (1-delta_atn)*FOIvdel + kappa + mu)*Sv[i])

dim(dEv) <- c(deltarp1, spor_len)
dEv[1,1] <- dt * (kappa*Ev[deltarp1] + (1-delta_atn)*FOIvdel*Sv[1] - (av*delta_atn + rho + mu)*Ev[1,1])
