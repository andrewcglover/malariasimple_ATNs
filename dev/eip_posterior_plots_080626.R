...
hill_r_all <- list_hill$r_all

hill_r_all_diff <- hill_r_all - hill_r_all[,dim(hill_r_all)[2]]

hill_r_all_pc <- 100 * hill_r_all_diff / hill_r_all[,dim(hill_r_all)[2]]

hill_r_all_diff_summary <- data.frame(
  x      = seq_len(ncol(hill_r_all_diff)),   # replace with your actual x vector if you have one
  median = apply(hill_r_all_diff, 2, median, na.rm = TRUE),
  lwr    = apply(hill_r_all_diff, 2, quantile, probs = 0.025, na.rm = TRUE),
  upr    = apply(hill_r_all_diff, 2, quantile, probs = 0.975, na.rm = TRUE)
)

ggplot(hill_r_all_diff_summary, aes(x = x)) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.2) +
  geom_line(aes(y = median), linewidth = 1) +
  labs(
    x = "Time",
    y = "Difference",
    title = "Posterior median and 95% CrI"
  ) +
  theme_classic()


hill_r_all_diff_summary %>% filter(x == 121 + (10*0))

hill_r_all_pc_summary <- data.frame(
  x      = seq_len(ncol(hill_r_all_pc)),   # replace with your actual x vector if you have one
  median = apply(hill_r_all_pc, 2, median, na.rm = TRUE),
  lwr    = apply(hill_r_all_pc, 2, quantile, probs = 0.025, na.rm = TRUE),
  upr    = apply(hill_r_all_pc, 2, quantile, probs = 0.975, na.rm = TRUE)
)

hill_r_all_pc_summary %>% filter(x == 121 + (10*0))

x_vals <- seq(-12, 12, by = 0.1)

length(x_vals)
# [1] 241

tail_idx <- which(x_vals >= 0)

x_tail <- x_vals[tail_idx]
y_tail_mat <- hill_r_all_diff[, tail_idx, drop = FALSE]

x_day <- 3.5

x_idx <- which.min(abs(x_vals - x_day))

y_summary <- quantile(
  hill_r_all_diff[, x_idx],
...
crossing_day <- apply(y_tail_mat, 1, function(y) {

  ok <- is.finite(y) & is.finite(x_tail)
  y <- y[ok]
  x <- x_tail[ok]

  if (length(y) < 2) return(NA_real_)

  # Check whether this trajectory reaches the threshold
  if (y_target > max(y) || y_target < min(y)) {
    return(NA_real_)
  }

  # y decreases as x increases, so reverse y/x for interpolation
  approx(
    x = rev(y),
    y = rev(x),
    xout = y_target
