...
dSv <- dt * (betaa - (FOIvdel + mu)*Sv)

dim(dEv) <- spor_len
dEv[1] <- dt * (FOIvdel*Sv - kappa*Ev[1] - mu*Ev[1])
dEv[2:spor_len] <- dt * (kappa*Ev[i-1] - kappa*Ev[i] - mu*Ev[i])

dIv <- dt * (kappa*Ev[spor_len] - mu*Iv)

update(Sv) <- if(Sv + dSv < 0) 0 else Sv + dSv
#update(Ev[1:spor_len]) <- if (Ev[i] + dEv[i] < 0) 0 else Ev[i] + dEv[i]
update(Ev[]) <- if (Ev[i] + dEv[i] < 0) 0 else Ev[i] + dEv[i]
update(Iv) <- if(Iv + dIv < 0) 0 else Iv + dIv
