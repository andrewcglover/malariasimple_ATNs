# --- Run both models for n_steps (days) ---
...
params_det <- get_parameters(n_days = n_steps) |>
  set_equilibrium(init_EIR = 10)
...
# Extract model output (assuming out_atn is a data.frame or matrix)
Q_df <- as.data.frame(out_atn)

# Basic time series plot
plot(Q_df$time, Q_df$Q_atn, type = "l", lwd = 2,
     col = "darkblue", xlab = "Time (days)", ylab = "ATN coverage (Q_atn)",
     main = "Proportion of Population Using ATNs Over Time")

# Add campaign marker
abline(v = params_atn$t0_atn, col = "red", lty = 2)
text(params_atn$t0_atn, max(Q_df$Q_atn, na.rm = TRUE)*0.9,
     "ATN distribution", pos = 4, col = "red")
