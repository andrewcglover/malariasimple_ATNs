...
set.seed(123)
rand_draws <- ceiling(runif(n_draws)*1000)
...
# ------------------------------------------------
only_draws <- only_pars |> filter(draw %in% rand_draws)
pbo_draws <- pbo_pars |> filter(draw %in% rand_draws)
cfp_draws <- cfp_pars |> filter(draw %in% rand_draws)
...
for (eir in eir_levels) {
  ...
  for (res in res_levels) {
    ...
    for (drawx in rand_draws) {

      # ONLY
      pars_only <- only_draws |> filter(resistance == res & draw == drawx)
      df_only <- run_atn_simulation(
        ...
        mutate(
          ...
          atn_action          = "Both",
          inhib_action        = "Long",
          resistance = res, EIR = eir, draw = drawx,
          pfpr2to10 = n_detect_730_3650 / n_730_3650
        )
      df_only$base_clin_inc_0_Inf <- df_base$n_clin_inc_0_Inf

      # PBO
      pars_pbo <- pbo_draws |> filter(resistance == res & draw == drawx)
      ...
      # CFP
      pars_cfp <- cfp_draws |> filter(resistance == res & draw == drawx)
...
df_plot <- bind_rows(all_runs) |>
  mutate(avert_clin_inc_0_Inf = base_clin_inc_0_Inf - n_clin_inc_0_Inf)


df_plot_pfpr <- df_plot |>
  group_by(time, itn_type, atn_action, inhib_action, resistance, EIR) |>
  summarise(
    pfpr2to10_me = median(pfpr2to10, na.rm = TRUE),
    pfpr2to10_lo = quantile(pfpr2to10, probs = 0.025, na.rm = TRUE),
    pfpr2to10_hi = quantile(pfpr2to10, probs = 0.975, na.rm = TRUE),
    .groups = "drop"
  )
...
df_plot_pfpr |>
  ...
  ggplot(
    aes(
      x = year, y = pfpr2to10_me, ymin = pfpr2to10_lo, ymax = pfpr2to10_hi,
      color = itn_type, fill = itn_type
      #linetype = atn_action,
      #alpha = inhib_action
      )
    ) +
